...
export default function AgencyFAQ() {
  ...
  const faqs = t("agency.faq.items", { returnObjects: true }) as Array<{ q: string; a: string }>;

  useEffect(() => {
    const ctx = gsap.context(() => {
      const mm = gsap.matchMedia();
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        gsap.from("[data-afaq-title]", { y: 30, autoAlpha: 0, duration: 0.7, ease: "power3.out", scrollTrigger: { trigger: "[data-afaq-heading]", start: "top 85%", once: true } });
        gsap.from("[data-afaq-item]", { y: 30, autoAlpha: 0, duration: 0.5, stagger: 0.08, ease: "power2.out", scrollTrigger: { trigger: "[data-afaq-list]", start: "top 80%", once: true } });
      });
      mm.add("(prefers-reduced-motion: reduce)", () => { gsap.set("[data-afaq-title], [data-afaq-item]", { autoAlpha: 1, clearProps: "all" }); });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="py-24" style={{ backgroundColor: "#f7f9fc" }}>
      <div className="max-w-3xl mx-auto px-6">
        <div data-afaq-heading className="text-center mb-16">
          <h2 data-afaq-title className="text-3xl md:text-4xl font-bold" style={{ visibility: "hidden" }}>{t("agency.faq.title")}</h2>
        </div>
        <div data-afaq-list className="flex flex-col gap-3">
          {faqs.map((faq, i) => (
            <div key={i} data-afaq-item className="bg-white rounded-xl border border-black/8 overflow-hidden" style={{ visibility: "hidden" }}>
