  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      mm.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        tl.from("[data-hero-mockup]", { y: 100, rotationX: 8, autoAlpha: 0, duration: 1, ease: "power2.out" }, "-=0.5");
        gsap.utils.toArray<HTMLElement>("[data-hero-shape]").forEach((shape, i) => {
          gsap.to(shape, { y: (i + 1) * -40, ease: "none", scrollTrigger: { trigger: sectionRef.current, start: "top top", end: "bottom top", scrub: 1 } });
        });
      });
      mm.add("(prefers-reduced-motion: reduce)", () => {
        gsap.set("[data-hero-badge], [data-hero-line], [data-hero-sub], [data-hero-cta], [data-hero-note], [data-hero-mockup]", { autoAlpha: 1, clearProps: "all" });
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-white" style={{ perspective: "1200px" }}>
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        ...
        <div data-hero-shape className="absolute top-10 -left-20 w-72 h-72 rounded-full opacity-8" style={{ backgroundColor: "#94BF5C" }} />
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-14 md:py-20 flex flex-col items-center text-center gap-6">
        <div data-hero-badge className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold text-white" style={{ backgroundColor: "#5D92E8", visibility: "hidden" }}>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
          ...
          <div className="flex flex-col gap-4 max-w-xl md:text-left text-center">
            <h1 className="text-3xl md:text-5xl font-bold leading-tight">
              <span data-hero-line className="block" style={{ clipPath: "inset(0 0 0 0)" }}>{t("hero.headlineOne")}</span>
              <span data-hero-line className="block" style={{ color: "#94BF5C", clipPath: "inset(0 0 0 0)" }}>{t("hero.headlineTwo")}</span>
            </h1>
        ...
        <p data-hero-note className="text-sm text-black/40 font-light" style={{ visibility: "hidden" }}>{t("hero.note")}</p>
              ...
              {[
                ...
                <div key={c.net} className="bg-white rounded-xl p-3 border border-black/8 flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <img src={c.logo} alt={c.net} className="w-4 h-4" />
        ...
        <a data-hero-cta href="#how-it-works" className="inline-flex items-center gap-2 px-7 py-3 rounded-full font-semibold text-sm border border-black/20 hover:bg-black/5 transition-colors" style={{ visibility: "hidden" }}>
