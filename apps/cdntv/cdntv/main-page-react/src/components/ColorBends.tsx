...
export default function ColorBends({
  ...
  rafRef.current = requestAnimationFrame(loop) as any;
    ...
    return () => {
      if (rafRef.current !== null) cancelAnimationFrame(rafRef.current as any);
      if (resizeObserverRef.current && typeof resizeObserverRef.current.disconnect === 'function') resizeObserverRef.current.disconnect();
  ...
  useEffect(() => {
  const material = materialRef.current as THREE.ShaderMaterial | null;
  const renderer = rendererRef.current as THREE.WebGLRenderer | null;
  if (!material) return;
