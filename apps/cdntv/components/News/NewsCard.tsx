...
export function NewsCard({
    ...
    return (
        <Link
            href={`/news/${slug}`}
            className="group cursor-pointer block bg-[#111] rounded-xl overflow-hidden border border-white/[0.06] hover:border-orange-400/30 transition-all duration-300 flex flex-col"
        ...
        >
            ...
            >
                <img
                    src={imageUrl}
                    alt={title}
                    style={{ width: '100%', height: '100%', objectFit: 'cover', maxWidth: 'none', display: 'block', transition: 'transform 500ms ease', pointerEvents: 'auto' }}
                    className="group-hover:scale-[1.03]"
                />
                <div
                    className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#111] to-transparent"
                    style={{ height: '4rem' }}
                />
            </div>
