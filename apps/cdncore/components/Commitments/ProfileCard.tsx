...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const dimensionsCache = useRef<{ width: number; height: number; wrapWidth: number; wrapHeight: number } | null>(null);
  ...
  const animationHandlers = useMemo(() => {
    ...
    const updateDimensions = (card: HTMLElement, wrap: HTMLDivElement) => {
    ...
    const updateCardTransform = (offsetX: number, offsetY: number, card: HTMLElement, wrap: HTMLDivElement) => {
      if (!dimensionsCache.current) updateDimensions(card, wrap);
      const { width, height } = dimensionsCache.current!;
    ...
    const createSmoothAnimation = (duration: number, startX: number, startY: number, card: HTMLElement, wrap: HTMLDivElement) => {
      ...
      if (!dimensionsCache.current) updateDimensions(card, wrap);
      const { wrapWidth, wrapHeight } = dimensionsCache.current!;
      const targetX = wrapWidth / 2;
      const targetY = wrapHeight / 2;
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => { dimensionsCache.current = null; });
    resizeObserver.observe(card);
    resizeObserver.observe(wrap);

    if (!dimensionsCache.current) animationHandlers.updateDimensions(card, wrap);
    const { wrapWidth } = dimensionsCache.current!;
    const initialX = wrapWidth - ANIMATION_CONFIG.INITIAL_X_OFFSET;
  ...
  const cardStyle = useMemo(
    ...
    [iconUrl, showBehindGradient, behindGradient, innerGradient]
  );
  ...
  interface IconPosition { x: string; y: string; size: string; }

  const iconPositions = useMemo((): IconPosition[] => {
    const grid = [[0, 2, 4], [1, 3, 5], [0, 2, 4], [1, 3, 5]];
    const columns = 6;
    const rows = 4;
    const cellWidth = 100 / columns;
    const cellHeight = 100 / rows;
    const iconSize = Math.min(cellWidth, cellHeight) * 0.7;
    const positions: IconPosition[] = [];
    grid.forEach((rowCols, rowIndex) => {
      rowCols.forEach((colIndex) => {
        positions.push({
        ...
        <div className="pc-inside">
          ...
          <div className="pc-content pc-avatar-content">
            ...
            <img
              ...
              loading="lazy"
              style={{ transform: `translateX(calc(-50% + ${avatarOffsetX}px)) translateY(${avatarOffsetY}px) scale(${avatarScale})` }}
