...
const Cubes = ({
  ...
  const tiltAt = useCallback((rowCenter: number, colCenter: number) => {
    ...
    sceneRef.current.querySelectorAll('.cube').forEach(cube => {
      ...
      if (dist <= radius) {
        const pct = 1 - dist / radius; const angle = pct * maxAngle;
  ...
  const onPointerMove = useCallback((e: PointerEvent) => {
    ...
    let w = dimensionsRef.current.width; let h = dimensionsRef.current.height;
    if (w === 0 || h === 0) { const r = sceneRef.current.getBoundingClientRect(); w = r.width; h = r.height; dimensionsRef.current.width = w; dimensionsRef.current.height = h; }
    const rect = sceneRef.current.getBoundingClientRect();
    const colCenter = (e.clientX - rect.left) / (w / gridSize);
    const rowCenter = (e.clientY - rect.top) / (h / gridSize);
  ...
  const onClick = useCallback((e: MouseEvent | TouchEvent) => {
    if (!rippleOnClick || !sceneRef.current) return;
    let w = dimensionsRef.current.width; let h = dimensionsRef.current.height;
    if (w === 0 || h === 0) { const r = sceneRef.current.getBoundingClientRect(); w = r.width; h = r.height; }
    const rect = sceneRef.current.getBoundingClientRect();
    const cellW = w / gridSize; const cellH = h / gridSize;
    const clientX = 'clientX' in e ? e.clientX : e.touches[0].clientX;
    const clientY = 'clientY' in e ? e.clientY : e.touches[0].clientY;
    const colHit = Math.floor((clientX - rect.left) / cellW);
    const rowHit = Math.floor((clientY - rect.top) / cellH);
    const spreadDelay = 0.15 / rippleSpeed; const animDuration = 0.3 / rippleSpeed; const holdTime = 0.6 / rippleSpeed;
  ...
  useEffect(() => {
    ...
    const loop = () => {
      if (!userActiveRef.current) {
        ...
        pos.x += (tgt.x - pos.x) * speed; pos.y += (tgt.y - pos.y) * speed;
        tiltAt(pos.y, pos.x);
  ...
  const wrapperStyle: React.CSSProperties & Record<string, string> = {
    '--cube-face-border': borderStyle, '--cube-face-bg': faceColor,
