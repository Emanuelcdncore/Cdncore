...
const AntigravityInner = ({
  ...
  useFrame((state) => {
    ...
    particles.forEach((particle, i) => {
      ...
      const proj = 1 - cz / 50;
      const ptx = targetX * proj;
      const pty = targetY * proj;

      const dx = mx - ptx;
      const dy = my - pty;
      const dist = Math.hypot(dx, dy);

      let pos = { x: mx, y: my, z: mz * depthFactor };
      ...
      if (dist < magnetRadius) {
        ...
        pos.x = ptx + r * Math.cos(angle);
        pos.y = pty + r * Math.sin(angle);
        pos.z = mz * depthFactor + Math.sin(t) * waveAmplitude * depthFactor;
      }

      particle.cx += (pos.x - particle.cx) * lerpSpeed;
      particle.cy += (pos.y - particle.cy) * lerpSpeed;
      particle.cz += (pos.z - particle.cz) * lerpSpeed;
      ...
      const distFromRing = Math.abs(
        Math.hypot(particle.cx - ptx, particle.cy - pty) - ringRadius
      );
