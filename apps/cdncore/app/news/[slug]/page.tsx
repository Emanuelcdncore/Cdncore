...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    const row = await getNewsBySlug('cdncore', slug);
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!row) {
        return (
            <div style={{ minHeight: '100vh', backgroundColor: '#0c0c14', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fff', marginBottom: '1rem' }}>Article not found</h1>
                    <a href="/news" style={{ color: '#c084fc' }}>Back to News</a>
                </div>
            </div>
        );
    }
