...
useGSAP(() => {
  ...
  dependencies: [endX],
  scope: container,
  revertOnUpdate: true  // causes the context to be reverted and the cleanup function to run every time the hook re-synchronizes
});
```
...
useGSAP((context, contextSafe) => {
  ...
  // ❌ DANGER! This animation is created in an event handler that executes AFTER useGSAP() executes.
  // It's not added to the context so it won't get cleaned up (reverted).
