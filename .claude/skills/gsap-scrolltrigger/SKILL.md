...
**start** / **end**: viewport position vs. trigger position. Format `"triggerPosition viewportPosition"`. Examples: `"top top"`, `"center center"`, `"bottom 80%"`, or numeric pixel value like `500` means when the scroller scrolls a total of 500px from the top. Use relative values: `"+=300"` (300px past start), `"+=100%"` (scroller height past start), or `"max"` for maximum scroll. Wrap in **clamp()** (v3.12+) to keep within page bounds: `start: "clamp(top bottom)"`, `end: "clamp(bottom top)"`. Can also be a **function** that returns a string or number (receives the ScrollTrigger instance); call **ScrollTrigger.refresh()** when layout changes.
...
| **containerAnimation** | Tween \| Timeline | For "fake" horizontal scroll: the timeline/tween that moves content horizontally. Pinning and snapping are not available on containerAnimation-based ScrollTriggers. |
...
- **batchMax** (Number | Function) — Max elements per batch. When full, the callback fires and the next batch starts.
...
- **pinSpacing** — default `true`; adds spacer element so layout doesn't collapse. Set `pinSpacing: false` only when layout is handled separately.
...
A common pattern: **pin** a section, then as the user scrolls **vertically**, content inside moves **horizontally** ("fake" horizontal scroll). The horizontal tween/timeline **must** use **ease: "none"**.
...
**Caveats:** Pinning and snapping are not available on ScrollTriggers that use **containerAnimation**. The container animation must use **ease: "none"**.

## Refresh and Cleanup

- **ScrollTrigger.refresh()** — recalculate positions (e.g. after DOM/layout changes, fonts loaded, or dynamic content). Automatically called on viewport resize, debounced 200ms.
...
- ✅ Create ScrollTriggers in the order they appear on the page (top to bottom). When created in a different order, set **refreshPriority** on each so they are refreshed in top-to-bottom order.

## Do Not

- ❌ Put ScrollTrigger on a **child tween** when it's part of a timeline; put it on the **timeline** or a **top-level tween** only.
- ❌ Forget to call **ScrollTrigger.refresh()** after DOM/layout changes that affect trigger positions; viewport resize is auto-handled, but dynamic content is not.
...
- ❌ Use an ease other than **"none"** on the horizontal animation when using **containerAnimation** for fake horizontal scroll.
